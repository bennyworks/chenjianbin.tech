...
import { TimeSlotMenu } from '@/components/course/time-slot-menu'
  ...
  return (
    <div className="p-4">
      <div className="flex gap-4 mb-4">
        <Button variant="outline" className="w-48 font-bold h-[60px] bg-gray-100">
          节次
        </Button>
        {days.map((day, index) => (
          <Button
          ...
          <div key={slot.id} className="flex gap-4">
            ...
            {days.map((day, index) => (
              ...
              >
                {courses
                  .filter(
                    (course) => course.timeSlotId === slot.id && course.dayOfWeek === index + 1
                  )
                  .map((course) => (
